...
public class CanvasController {
		...
		timer.scheduleAtFixedRate(new TimerTask() {
			...
			public void run() {
				...
				Platform.runLater(() -> {
					...
					try {
						List<Classification> currentPredictions = model.getPredictions(getCurrentSnapshot(), 10);
	...
	/**
	 * This is a helper method that builds a string of the current top 10
	 * predictions, which can be displayed in a label. Note that it also checks
	 * whether the player has won (the top 3 predictions contain the category to be
	 * drawn).
	 * 
	 * @param currentPredictions a list of of type Classification, which is the
	 *                           category of AI predictions
	...
	private String getPredictionDisplay(List<Classification> currentPredictions) {
		...
		for (final Classifications.Classification classification : currentPredictions) {
			...
			sb.append("TOP ").append(predictionRank).append(" : ")
